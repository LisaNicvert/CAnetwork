...
# Date: 2024-05-07
#
# Script Description: simulation function for interaction networks

#' Get niches
#'
#' Returns the fundamental and realized niches and the niches returned by reciprocal scaling.
#'
#' @param rec A dataframe (expected to be the output of `reciprocal.coa` function).
...
#' @param comm The observed community matrix. It is used to get the sites/resource species
#' alpha diversity/niche width.
#' @param consumer_niche A nx2x2 array. The last 2 dimensions correspond to the 2 traits.
#' There are n rows for n species. The middle dimension corresponds to the species niche optima
#' and standard deviation corresponding to its multivariate normal niche.
#' @param resource_traits A mx2 matrix. Each row corresponds to a species and each column to a trait value.
#' @param rowname How to name the first element of the list (resources/rows) (eg could be "plants")
#' @param colname How to name the second element of the list (resources/rows) (eg could be "birds")
#'
...
#' For resource and consumers we have 2 lists:
#'
#' + mean: niche optima
#' + sd: niche breadths
#'
...
#' + mvar: niche parameters corresponding to the mean and sd given by reciprocal scaling.
#'
#' These matrices describe the value for each species (in rows) along each trait/axis (in column).
#'
#' @details
#' The fundamental niche optimum of a consumer is the mean of its niche (from the `consumer_niche` table)
#' and for a resource it corresponds to the traits from the `resource_traits` table.
#' The fundamental niche breadths are computed for consumers only and corresponds to the breadths
#' of their niche (from the `consumer_niche` table).
#'
#' The realized niche optima are computed as the weighted means of the traits of species that a
#' species interact with. The realized niche of a consumer is the weighted mean of the traits
#' of the resource species it interacts with, and reciprocally.
#' The realized niche breadths are computed as the weighted standard deviations of the traits of
#' species that a species interact with. The realized niche breadth of a consumer is the
#' weighted standard deviation of the traits of the resource species it interacts with,
#' and reciprocally.
#'
#' The niche optima computed with multivariate methods correspond to the reciprocal scaling mean.
#' The niche widths correspond to the reciprocal scaling standard deviation.
#'
#' @export
...
#' + type: the type of individuals for which the measure is (rows or columns?). The column
#' and row codes are chosen with `rowname` and `colname`.
